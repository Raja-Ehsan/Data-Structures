...
void movie_country(string name, MovieNode* nl) {
	if (nl == NULL)
		return;
	if (nl->data.title == name)
	{
		cout << nl->data.country << endl;
	}
	else if (nl->data.title > name)
		movie_country(name, nl->lchild);
	else
		movie_country(name, nl->rchild);
	return;
}

//-------------------------------------------------------------------------------------------
//-------------------------------------------------------------------------------------------
void movie_rates(MovieNode* nl, RatingsNode* rl, int votes)
{
	if (rl == NULL)
		return;

	if (rl->data.total_votes >= votes)
		cout << nl->data.title << endl;

	else if (rl->data.imdb_title_id > nl->data.title_id)
		movie_rates(nl, rl->lchild, votes);
	else
		movie_rates(nl, rl->rchild, votes);

}
void movie_ratings(MovieNode* nl, RatingsNode* rl, int votes) {//output all movies with votes above given total votes
	if (nl == NULL)
		return;
	movie_rates(nl, rl, votes);
	movie_ratings(nl->rchild, rl, votes);
	movie_ratings(nl->lchild, rl, votes);
}
//-------------------------------------------------------------------------------------------
//-------------------------------------------------------------------------------------------


void traverse_movie_node(string name, title_principles_Node* roottitle, MovieNode* rootmovie)
{
	if (rootmovie == NULL)
		return;
	if (rootmovie->data.title_id == roottitle->data.imdb_title_id)
	{
		if (rootmovie->data.director == name)
			cout << "The movie for entered director is : " << rootmovie->data.title;
	}
	else if (rootmovie->data.title_id > roottitle->data.imdb_title_id)
		traverse_movie_node(name, roottitle, rootmovie->lchild);
	else
		traverse_movie_node(name, roottitle, rootmovie->rchild);
	return;
}
void traverse_titleprincipals(string name, NameNode* rootname, title_principles_Node* roottitle, MovieNode* rootmovie)
{
	if (roottitle == NULL)
		return;
	if (rootname->data.name_id == roottitle->data.imdb_name_id)
	{
		traverse_movie_node(name, roottitle, rootmovie);
	}
	else if (rootname->data.name_id > roottitle->data.imdb_name_id)
		traverse_titleprincipals(name, rootname, roottitle->rchild, rootmovie);
	else
		traverse_titleprincipals(name, rootname, roottitle->lchild, rootmovie);
	return;
}
void director_movies(string name, NameNode* rootname, title_principles_Node* roottitle, MovieNode* rootmovie)
{
	if (rootname == NULL)
		return;
	if (name == rootname->data.name)
	{
		traverse_titleprincipals(name, rootname, roottitle, rootmovie);
	}
	director_movies(name, rootname->rchild, roottitle, rootmovie);
	director_movies(name, rootname->lchild, roottitle, rootmovie);
}
...
void main() {
	...
	cout << "The country for movie input is : \n\n";
	movie_country("L'Inferno", ml.root);

	cout << "\n\nMovie with votes more than input are : \n\n" << endl;
	movie_ratings(ml.root, rl.root, 150);

	cout << "\n\nDirector movies : " << endl;
	director_movies("Alexander Black", n1.root, t1.root, ml.root);
	getchar();
	getchar();
}